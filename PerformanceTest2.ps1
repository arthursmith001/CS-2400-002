...
function Start-SpeedupComparison {
    ...
    elseif ($inputMethod -eq "2") {
        ...
        do {
            ...
            # Validate CSV format (Removed Processors check)
            $headers = Get-Content $csvPath -First 1
            if ($headers -notmatch '(?i)Machine.*ClockSpeed.*Instructions.*Cycles.*CPI') { # Removed Processors
                Write-Host "Error: Invalid CSV format. Required headers: Machine, ClockSpeed, Instructions, Cycles, CPI" -ForegroundColor Red # Corrected message
                continue
            }
            break
        } while ($true)
...
foreach ($machine in $machines) {
    $executionTimes = $machine.Group | ForEach-Object {
        [double]$script:cycles = $_.Cycles # Use $script: to access the script-level variable
        [double]$script:clockSpeed = $_.ClockSpeed # Use $script: to access the script-level variable
        if ($script:clockSpeed -eq 0) {
            Write-Warning "ClockSpeed is zero for $($_.Machine). Skipping this entry."
            return
        }
        $script:cycles / $script:clockSpeed
    }

                $baselineTime = ($executionTimes | Measure-Object -Maximum).Maximum

                if ($baselineTime -eq 0) {
                    Write-Warning "Baseline time is zero for $($machine.Name). Skipping this machine."
                    continue
                }

                $optimizedTime = ($executionTimes | Measure-Object -Minimum).Minimum

                # Get processors (now optional, default to 1 if not present)
                $processors = 1 # Default if not in CSV
                if ($machine.Group | Select-Object -First 1 | Get-Member -Name Processors -MemberType NoteProperty) { # Check if it exists
                    ...
                    if ($processors -eq 0) { 
                        ...
                        $processors = 1 # Set to 1 if it is 0.
                    }
                } else {
